...
namespace Day_14_Repeat
{
    public class SampleScript : IExternalScript
    {
        ...
        public OptimProperty levelSize = new OptimProperty(10, 10, 100, 10);
        public OptimProperty PosIncrement = new OptimProperty(10, 10, 100, 10);
        public OptimProperty PosPctOfDepo = new OptimProperty(50, 10, 100, 10);
        ...
        public void Execute(IContext ctx, ISecurity sec)
        {
            ...
            for (int i = start; i < count; i++)
            {
                ...
                var profitPcnt = (totalProfit / sec.InitDeposit) * 100;
                var oldLevel = currentLevel;
                currentLevel = (int)(profitPcnt / levelSize);
                var poseMoney = sec.InitDeposit * PosPctOfDepo / 100.0 + currentLevel * (PosIncrement / 100.0 * sec.InitDeposit);
                var maxShares = (int)(poseMoney / (sec.Bars[i].Close * sec.LotSize));

                if (oldLevel!=currentLevel)
                    ctx.LogInfo("{3} CurrentLevel: {0}->{1}:{2}", oldLevel, currentLevel, poseMoney, i);
                
                if (le == null)
                {
                    sec.Positions.BuyIfGreater(i + 1, maxShares, highChanel[i], 50, "LE");
                    stopMoveFlag = false;
                    trEnableFlag = false;
                }
                else
                {
                    ...
                    if (leAdd == null && leD == null)
                    {
                        ...
                        sec.Positions.BuyIfGreater(i + 1, le.Shares, posAddPrice, "LEadd");
                    }
